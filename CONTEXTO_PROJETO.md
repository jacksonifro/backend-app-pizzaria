...
│   │   └── DetalheItemController.ts
│   ├── Pedido/
│   │   ├── CreatePedidoController.ts
│   │   ├── DeletarPedidoController.ts
│   │   ├── DetalhesPedidoController.ts
│   │   ├── EnviarPedidoController.ts
│   │   ├── FinalizarPedidoController.ts
│   │   └── ListPedidoController.ts
│   ├── Produto/
│   │   ├── CreateProductController.ts
│   │   ├── DeleteProductController.ts
│   │   ├── DetalhesProductController.ts
│   │   ├── EditProductController.ts
│   │   ├── ListProductCatController.ts
│   │   └── ListProductController.ts
│   └── Usuario/
│       ├── AutenticarUserController.ts
│       ├── CreateUserController.ts
│       ├── DetalhesUserController.ts
│       └── EditUserController.ts
├── generated/
│   └── prisma/
│       └── ...
├── middlewares/
│   ├── autenticarTokenUser.ts
│   ├── verificarPerfilAdmin.ts
│   └── validateSchema.ts
├── prisma/
│   └── index.ts
├── schemas/
...
│   ├── Item/
│   │   ├── CreateItemService.ts
│   │   ├── DeleteItemService.ts
│   │   └── DetalhesItemService.ts
│   ├── Pedido/
│   │   ├── CreatePedidoService.ts
│   │   ├── DeletePedidoService.ts
│   │   ├── DetalhesPedidoService.ts
│   │   ├── EnviarPedidoService.ts
│   │   ├── FinalizarPedidoService.ts
│   │   └── ListPedidoService.ts
│   ├── Produto/
│   │   ├── CreateProductService.ts
│   │   ├── DeleteProductService.ts
│   │   ├── DetalhesProductService.ts
│   │   ├── EditProductService.ts
│   │   ├── ListProductCatService.ts
│   │   └── ListProductService.ts
│   └── Usuario/
│       ├── AutenticarUserService.ts
│       ├── CreateUserService.ts
│       ├── DetalhesUserService.ts
│       └── EditUserService.ts
├── router.ts
└── server.ts
...
### 1. `autenticarTokenUser` (`middlewares/autenticarTokenUser.ts`)

**Função**: Valida se o usuário está autenticado verificando o token JWT e adiciona `user_id` ao `req`.

**Fluxo**:

1. Extrai o token do header `Authorization: Bearer <token>`
2. Verifica a validade do token usando `jsonwebtoken`
3. Extrai o `sub` (user id) do payload do token
...
```typescript
router.get("/me", autenticarTokenUser, new DetalhesUserController().handle);
```
...
### 2. `verificarPerfilAdmin` (`middlewares/verificarPerfilAdmin.ts`)

**Função**: Verifica se o usuário autenticado tem permissão de `ADMIN`.

**Pré-requisito**: Deve ser usado **após** o middleware `autenticarTokenUser`.

**Fluxo**:

1. Obtém `user_id` do `req` (adicionado pelo `autenticarTokenUser`)
2. Busca o usuário no banco de dados (via Prisma)
3. Verifica se o campo `perfil` é igual a `"ADMIN"`
...
```json
{
  ...
  "details": [
    { "campo": "name", "mensagem": "O nome precisa ter no minimo 3 letras" },
    { "campo": "email", "mensagem": "Precisa ser um email valido" }
  ]
}
```
...
- `AutenticarUserController` - Autentica usuário (login)
- `DetalhesUserController` - Retorna dados do usuário logado
- `EditUserController` - Edita dados do usuário
...
- `DetalhesProductController` - Detalhes de um produto
- `EditProductController` - Edita produto
- `ListProductCatController` - Lista produtos por categoria

#### Pedido

- `CreatePedidoController` - Cria novo pedido
- `EnviarPedidoController` - Envia/Confirma pedido (sai do rascunho)
- `FinalizarPedidoController` - Finaliza pedido
- `ListPedidoController` - Lista pedidos (filtro por rascunho)
- `DetalhesPedidoController` - Detalhes de um pedido
- `DeletarPedidoController` - Deleta pedido permanentemente

#### Itens (Pedido_Item)

- `CreateItemController` - Adiciona item ao pedido
- `DeleteItemController` - Remove item do pedido
- `DetalheItemController` - Detalhes de um item
...
- `AutenticarUserService` - Lógica de autenticação
- `DetalhesUserService` - Lógica de detalhes do usuário
- `EditUserService` - Lógica de edição do usuário
...
- `DetalhesProductService` - Lógica de detalhes do produto
- `EditProductService` - Lógica de edição do produto
- `ListProductCatService` - Lógica de listagem por categoria

#### Pedido

- `CreatePedidoService` - Lógica de criação de pedido
- `EnviarPedidoService` - Lógica de envio/confirmar pedido
- `FinalizarPedidoService` - Lógica de finalização
- `ListPedidoService` - Lógica de listagem com filtro
- `DetalhesPedidoService` - Lógica de detalhes do pedido
- `DeletePedidoService` - Lógica de deleção permanente

#### Itens (Pedido_Item)

- `CreateItemService` - Lógica de adição de item
- `DeleteItemService` - Lógica de remoção de item
- `DetalhesItemService` - Lógica de detalhes do item
